...
class BoardWidget extends Pad {
     ...
     * row stuff.
     */
    private char[] _rows = new char[]{'a', 'b', 'c', 'd', 'e', 'f', 'g'};
    /**
     * Col stuff.
     */
    private char[] _cols = new char[]{'1', '2', '3', '4', '5', '6', '7'};
    ...
    public synchronized void paintComponent(Graphics2D g) {
        ...
        g.fillRect((_selectedCol - 'a') * SQDIM , (('7' - _selectedRow)) * SQDIM, SQDIM, SQDIM);
        for (char row : _rows) {
            for (char col : _cols) {
                x = (row - 'a') * SQDIM;
                y = (('7' - col)) * SQDIM;
                if (_model.get(row, col) == RED) {
                    g.setColor(RED_COLOR);
                    g.fillOval(x, y, PIECE_RADIUS * 2, PIECE_RADIUS * 2);
                } else if (_model.get(row, col) == BLUE) {
                    g.setColor(BLUE_COLOR);
                    g.fillOval(x, y, PIECE_RADIUS * 2, PIECE_RADIUS * 2);
                } else if (_model.get(row, col) == BLOCKED) {
                    drawBlock(g, x, y);
                }
                g.setColor(BLANK_COLOR);
            }
        ...
        for (int i = 0; i < 7; i++) {
            ...
            g.drawLine(i * SQDIM, 0, i * SQDIM, boardLen);
            g.setColor(LINE_COLOR);
        }
        for (int i = 0; i < 7; i++) {
            g.setStroke(LINE_STROKE);
            g.drawLine(0, i * SQDIM, boardLen, i * SQDIM);
            g.setColor(LINE_COLOR);
        }
        ...
        if (where.getButton() == MouseEvent.BUTTON1) {
            ...
            if (mouseCol >= 'a' && mouseCol <= 'g'
                ...
                if (_blockMode) {
                    ...
                    if (_selectedCol != 0) {
                        System.out.println("" + _selectedCol
                        ...
                        if (_model.legalMove(_selectedCol, _selectedRow,
                            ...
                            _commandQueue.offer("" + _selectedCol
                                    + _selectedRow + "-" + mouseCol + mouseRow);
                            _selectedCol = _selectedRow = 0;
                        } else if (("" + _selectedCol + _selectedRow
                                + "-" + mouseCol + mouseRow).equals("-")) {
                            _commandQueue.offer("-");
                        } else {
                            _selectedCol = _selectedRow = 0;
                        }
