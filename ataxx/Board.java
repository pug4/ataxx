...
class Board {
    ...
    PieceColor getWinner() {
        if (!canMove(_whoseMove)
            ...
            if (_numPieces[_whoseMove.opposite().ordinal()]
                    > _numPieces[_whoseMove.ordinal()]) {
                return _whoseMove.opposite();
            }
            if (_numPieces[_whoseMove.opposite().ordinal()]
                    < _numPieces[_whoseMove.ordinal()]) {
                return _whoseMove;
    ...
    boolean legalMove(Move move) {
        if (move != null) {
            ...
            if (get(move.fromIndex()) == null || get(move.toIndex()) == null) {
                return false;
            }
            ...
            for (int i = -1; i < 2; i++) {
                for (int j = -1; j < 2; j++) {
                    if (get(neighbor(move.toIndex(), i, j))
                        ...
                        _numPieces[_whoseMove.opposite().ordinal()] -= 1;
                    }
                }
            }
            _numJumps += 1;
    ...
    void undo() {
        while (!_undoSquares.isEmpty()) {
            ...
            Integer squ = _undoSquares.pop();
            PieceColor pc = _undoPieces.pop();
            incrPieces(get(squ), -1);
            incrPieces(pc, 1);
            unrecordedSet(squ, pc);
        }
        _numMoves -= 1;
        _whoseMove = _whoseMove.opposite();
    ...
    boolean legalBlock(char c, char r) {
        char tempColumn = (char) ('h' - (c - 'a' + 1));
        char tempRow = (char) ('8' - (r - '0'));
        if (get(tempColumn, tempRow) == EMPTY && get(c, r)
                == EMPTY && get(tempColumn, r) == EMPTY
                && get(c, tempRow) == EMPTY && _numMoves == 0) {
            return true;
        }
        return false;
    }
    ...
    void setBlock(char c, char r) {
        ...
        if (get(tempColumn, r) != BLOCKED) {
            unrecordedSet(tempColumn, r, BLOCKED);
            _totalOpen -= 1;
        }
        if (get(c, tempRow) != BLOCKED) {
            unrecordedSet(c, tempRow, BLOCKED);
            _totalOpen -= 1;
        }
        if (get(tempColumn, tempRow) != BLOCKED) {
            unrecordedSet(tempColumn, tempRow, BLOCKED);
            _totalOpen -= 1;
        }
    ...
    private char[] _cols;
    /**
     * number of moves that has occured.
     */
